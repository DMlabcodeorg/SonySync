...
def main():
    ...
    if opt_get.download_flag == 1:
        ...
        for classroom_pth in list_folders_with_string_in_name('./'):
            date = classroom_pth.split('_')
            month = int(date.split('-')[0])
            day = int(date.split('-')[1])
            year = date.split('-')[2]
            if (month > 6 and int(year) >= 2023) or (month == 6 and day >= 8 and int(year) == 2023):
                ...
                flag_freq = 3000
            command = 'python test_flag_class.py --input_path {} --output_path {} --flag_freq {}'.format(classroom_pth,
                                                                                                         classroom_pth,
                                                                                                         flag_freq)
            os.system(command)
